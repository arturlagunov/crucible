...
export class Decorator {
  ...
  constructor(private view: SessionView) {}
  ...
  decorate(editor: vscode.TextEditor | undefined): void {
    if (!editor || !this.view.bundle) {
      return;
    }
    const live = this.view.panel.liveFor(editor.document.uri);
    const marks: vscode.DecorationOptions[] = [];
    const hi: vscode.DecorationOptions[] = [];
    for (const ct of live) {
      const data = this.view.panel.dataOf(ct);
      if (!data) {
        continue;
      }
      const line = ct.range?.start?.line ?? data.lines[0] - 1;
      if (this.gutter) {
        marks.push({
      ...
      hi.push({ range: ct.range ?? data.range(editor.document) });
    }
    if (this.gutter) {
...
export { Decorator as EditorDecorator };
